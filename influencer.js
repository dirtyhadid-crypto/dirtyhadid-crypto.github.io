...
const TABLE_COLUMNS = [
  { key: "adGroup", label: "广告组", type: "text" },
  { key: "channel", label: "CHANNEL", type: "text" },
  { key: "publisher", label: "出版商", type: "text" },
  { key: "clicks", label: "点击量", type: "number" },
  { key: "dpv", label: "总 DPV", type: "number" },
  { key: "atc", label: "ATC 总计", type: "number" },
  { key: "revenue", label: "购买总额", type: "number" },
  { key: "units", label: "商品销量总计", type: "number" },
  { key: "bonus", label: "品牌引流奖励计划", type: "number" }
];
...
function renderSheetBoard() {
  ...
  dashboardState.sheets.forEach((sheet) => {
    ...
    const modeSelect = card.querySelector(".line-mode-select");
    const lineSelect = card.querySelector(".line-metric-select");
    const pieSelect = card.querySelector(".pie-metric-select");
    const sortTriggers = card.querySelectorAll(".sort-trigger");

    if (modeSelect) {
      modeSelect.value = sheet.lineMode;
      modeSelect.addEventListener("change", () => {
        sheet.lineMode = modeSelect.value === "all" ? "all" : "single";
    ...
    sortTriggers.forEach((button) => {
      button.addEventListener("click", () => {
        ...
        if (sheet.sortKey === key) {
          sheet.sortDirection = sheet.sortDirection === "desc" ? "asc" : "desc";
        } else {
          sheet.sortKey = key;
          sheet.sortDirection = getDefaultSortDirection(key);
        }
        renderSheetModule(sheet);
      });
    });
  ...
  return `
    <article class="sheet-card" data-sheet-id="${escapeHtml(sheet.id)}">
      ...
      <section class="sheet-controls">
        <label>
          折线图显示
          <select class="line-mode-select">
            <option value="single">单个变量</option>
...
function tableHeadMarkup() {
  const headerCells = TABLE_COLUMNS.map((column) => {
    return `
      <th>
        <button
          type="button"
...
function renderSheetModule(sheet) {
  ...
  const lineRows = sortRowsByMetric(sheet.rows, sheet.lineMetric);
  const tableRows = sortRowsByColumn(sheet.rows, sheet.sortKey, sheet.sortDirection);
  ...
  const lineMetricSelect = card.querySelector(".line-metric-select");

  if (lineMetricSelect) {
    lineMetricSelect.disabled = sheet.lineMode === "all";
  }

  if (lineTitle) {
    lineTitle.textContent = sheet.lineMode === "all"
      ? `${sheet.name}｜折线图变量：全部核心变量`
      : `${sheet.name}｜折线图变量：${getMetricLabel(sheet.lineMetric)}`;
  }
  ...
  if (tableNote) {
    tableNote.textContent = `默认展示前 5 行，其余折叠。当前排序：${getColumnLabel(sheet.sortKey)}（${sheet.sortDirection === "asc" ? "升序" : "降序"}）`;
  }

  updateSortHeaderIndicators(card, sheet);
  renderLineChart(sheet, lineRows, card.querySelector(".line-canvas"));
  renderPieChart(sheet, sheet.rows, card.querySelector(".pie-canvas"));
  renderSheetTable(card, tableRows);
}
...
function renderLineChart(sheet, rows, canvas) {
  ...
  const lineMetrics = sheet.lineMode === "all"
    ? METRIC_CONFIG.map((item) => item.key)
    : [sheet.lineMetric];

  const datasets = lineMetrics.map((metricKey) => ({
    label: getMetricLabel(metricKey),
    data: rows.map((row) => toNumber(row[metricKey])),
    borderColor: getLineColor(metricKey),
    backgroundColor: withAlpha(getLineColor(metricKey), 0.18),
    borderWidth: metricKey === sheet.lineMetric ? 2.8 : 2,
    pointRadius: metricKey === sheet.lineMetric ? 2.2 : 1.6,
    pointHoverRadius: metricKey === sheet.lineMetric ? 3.6 : 2.8,
    tension: 0.25,
    fill: lineMetrics.length === 1
  }));
...
function sortRowsByColumn(rows, sortKey, sortDirection) {
  const direction = sortDirection === "asc" ? 1 : -1;
  const column = TABLE_COLUMNS.find((item) => item.key === sortKey);
  ...
  return rows.slice().sort((a, b) => {
    if (type === "number") {
      ...
      const diff = textA.localeCompare(textB, "zh-CN");
      if (diff !== 0) {
        return diff * direction;
      }
    }

    return String(a.adGroup ?? "").localeCompare(String(b.adGroup ?? ""), "zh-CN");
  });
}

function updateSortHeaderIndicators(card, sheet) {
  const triggers = card.querySelectorAll(".sort-trigger");
  triggers.forEach((button) => {
    const indicator = button.querySelector(".sort-indicator");
    const key = button.dataset.sortKey || "";
    const isActive = key === sheet.sortKey;
    const state = isActive ? sheet.sortDirection : "none";
    button.dataset.sortState = state;

    if (indicator) {
      indicator.textContent = state === "asc" ? "↑" : state === "desc" ? "↓" : "↕";
    }
  });
}
...
function getColumnLabel(columnKey) {
  const matched = TABLE_COLUMNS.find((item) => item.key === columnKey);
  return matched ? matched.label : columnKey;
}

function getLineColor(metricKey) {
  return LINE_COLOR_MAP[metricKey] || "#5ce9ff";
}

function withAlpha(hex, alpha) {
  const normalized = String(hex || "").trim().replace("#", "");
  if (normalized.length !== 6) {
    return `rgba(92, 233, 255, ${alpha})`;
  }
